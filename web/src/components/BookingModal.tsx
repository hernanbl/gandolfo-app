...
const BookingModal = ({ open, onOpenChange }: BookingModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Usar Netlify Functions - funciona tanto en desarrollo como en producción
      ...
      const response = await fetch(`${API_URL}/.netlify/functions/send-booking-email`, {
        ...
        body: JSON.stringify({
          ...
          email,
          phone,
          service: company || "Consulta General", // Mapear company a service
          date: date ? format(date, "dd/MM/yyyy") : '', // Format as DD/MM/YYYY for the API
          time: selectedHour,
          message: "" // Campo opcional para SendGrid
        }),
      });
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <p className="text-sm text-muted-foreground mb-2">
                        Zona horaria: GMT-3 (Hora de Buenos Aires)
                      </p>
                    </div>
                  </div>
                </div>
